...
void main() {
  testWidgets('LeaderboardPage displays users correctly (functional check)', (tester) async {
    ...
    for (int i = 0; i < 35; i++) {
      final uid = 'user_$i';
      await firestore.collection('users').doc(uid).set({'displayName': 'User $i'});
      await firestore
          .collection('users')
          .doc(uid)
          .collection('summary')
    ...
    expect(find.text('User 0'), findsNothing); // Likely off-screen but should be in the list if we scroll
